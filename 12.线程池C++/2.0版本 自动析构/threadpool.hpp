...
{
    Task(); /* 无参构造 */
    Task(const Task<T> &ob); /* 拷贝构造 */
    Task(callback func, T *arg); /* 有参构造 */
    Task<T> &operator=(const Task<T> &ob); /* 拷贝赋值 */
    ~Task(); /* 析构函数 */
    
    callback function; /* 执行任务函数 */
    T *arg; /* 参数 */
    static int num;
};
...
template<typename T>
Task<T>::Task(const Task<T> &ob)/* 拷贝构造 */
{
    this->function = ob.function;
    this->arg = new T;
    memcpy(this->arg, ob.arg, sizeof(T));
//cout << "拷贝构造" << endl;
}

template<typename T>
Task<T>::Task(callback func, T *arg) /* 有参构造 */
{
    this->function = func;
    this->arg = (T *)arg;
    cout << "有参构造" << endl;
...
template<typename T>
Task<T> &Task<T>::operator=(const Task<T> &ob) /* 拷贝赋值 */
{
//cout << "--------拷贝赋值" << endl;
    if (this->arg != nullptr)
    {
...
public:
    ...
    void addTask(callback func, T *arg);

    //取出一个任务
    void getTask(Task<T> &task);
...
template<typename T>
TaskQueue<T>::TaskQueue() /* TaskQueue构造函数 */
{
    pthread_mutex_init(&mutexTask, NULL); /* 锁的初始化 */
}

template<typename T>
TaskQueue<T>::~TaskQueue() /* TaskQueue析构函数 */
{
    pthread_mutex_destroy(&mutexTask);
}

template<typename T>
void TaskQueue<T>::addTask(Task<T> &task) /* 添加任务函数 {Task} */
{
...
template<typename T>
void TaskQueue<T>::addTask(callback func, T *arg) /* 添加任务函数 {callback}{void *}*/
{
    pthread_mutex_lock(&mutexTask);
    Task<T> task;
    task.arg = arg;
...
template<typename T>
void TaskQueue<T>::getTask(Task<T> &task) /* 获取任务函数 */
{
    pthread_mutex_lock(&mutexTask);
    if (m_queue.size() > 0)
    {
...
public:
    ...
    void add_Task(callback func, T *arg); /* 添加任务 */
...
template<typename T>
ThreadPool<T>::~ThreadPool() /* ThreadPool析构函数 */
{
...
template<typename T>
int ThreadPool<T>::get_BusyNum()  /* 获取忙的线程数 */
{
    int busynum = 0;
...
template<typename T>
int ThreadPool<T>::get_AliveNum() /* 获取存活的线程数 */
{
    int alivenum = 0;
    pthread_mutex_lock(&mutexPool);
...
template<typename T>
void ThreadPool<T>::add_Task(callback func, T *arg) /* 添加任务 */
{
...
template<typename T>
void ThreadPool<T>::thread_Exit() /* 单个线程退出函数 */
{
...
template<typename T>
void *ThreadPool<T>::manager(void *arg) /* 管理者线程 */
{
...
cout << "liveNum = " << liveNum  << "  busyNum = " << busyNum << endl;
...
template<typename T>
void *ThreadPool<T>::worker(void *arg) /* 工作者线程 */
{
    ThreadPool *pool = static_cast<ThreadPool *>(arg);
...
template<typename T>
void ThreadPool<T>::handler(void *arg) /* 意外退出处理函数 */
{
    ThreadPool *pool = static_cast<ThreadPool *> (arg);
