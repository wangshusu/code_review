...
struct Task /* 任务结构体 */
{
    ...
    Task(callback func, void *arg):function(func), arg(arg){}
    // ~Task() // 析构函数 但是因为无法适配这个类型 这个析构函数不能存在
    // {
    //     if (arg != nullptr)
    //     {   
    //         delete arg;
...
void TaskQueue::addTask(Task &task) /* 添加任务函数 {Task} */
{
...
void TaskQueue::addTask(callback func, void *arg) /* 添加任务函数 {callback}{void *}*/
{
    pthread_mutex_lock(&mutexTask);
    Task task;
    task.arg = arg;
...
public:
    ...
    void add_Task(callback func, void *arg); /* 添加任务 */
...
ThreadPool::ThreadPool(int minNum, int maxNum) /* 线程池的构建函数 */
{
...
void *ThreadPool::worker(void *arg) /* 工作者线程 */
{
    ...
    while (true)   /* 工作者 线程轮询 */
    {
        ...
        Task task = pool->m_Task->getTask(); /* 获取任务 */
        pool->busyNum++;
        pthread_mutex_unlock(&pool->mutexPool); /* 先把线程池解锁 */
