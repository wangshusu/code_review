	...
	while(1)
	{
		...
		if(ret > 0)
		{
			...
			if (cmd == __CMD_LS)/* 如果是要文件列表 */
			{
				...
				Queue *qu_path = get_dir_path(__FTP_FILE_PATH, &qu_filename);//获取路径 文件名
				MyProtMsg *msg = server_Head_Init(__CMD_LS, qu_path->num);//服务器包的初始化
				uint8_t *file_name_p = (uint8_t *)&msg->file_haed.data;//强转结构体类型
			...
			else if (cmd == __CMD_GET)/* 如果是要获取文件 */
			{
				...
				MyProtMsg *msg = server_Head_Init(__CMD_LS, 0);//服务器包的初始化
				int fd = open(p[0],O_RDONLY);
				if(fd == -1)
				{
					msg->pag_head.result = 0;//服务端失败
					strcpy((uint8_t *)&ptr->file_haed.data, "文件不存\n");
					send(confd1, (void *)&msg, sizeof(MyProtMsg), 0);
				}
