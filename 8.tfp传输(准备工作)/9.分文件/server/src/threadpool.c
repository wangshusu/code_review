...
{
    printf("handler_Worker is woeker!\n");
...
for (int i = 0; i < pool->maxNum; i++)
{
    printf("WorkerID: %ld\n",pool->workersID[i]);
        ...
        if (liveNum < pool->maxNum && queueSize != 0)
        {
                ...
                && pool->liveNum < pool->maxNum; ++i)
            {
                if (pool->workersID[i] == 0) /* 那么就在这来创建 */
                {
                    ...
                    pthread_detach(pool->workersID[i]);
                    pool->liveNum++;    /* 生存的个数++ */
                    counter++;
                    printf("\n\nAdd pthread !\n\n");
                }
        ...
        /* 任务的添加 */
        /* 要求:忙的数量 < 存活的数量 && 排队数量不为0 && 线程池没有关闭 */
        // if (busyNum < liveNum && queueSize != 0 && !pool->shoutdown)
        // {
        //     pthread_cond_signal(&pool->isFull);    /* 唤醒线程 */
        // }
    }
...
void *worker_Function(void *arg)
{
    ...
    pthread_cleanup_push(handler_Worker,(void *)pool); /* 设置一个线程退出清理函数-->防止线程带锁退出 */
    while (1)   /* 工作者 线程轮询 */
    {
    ...
    // 正常死亡的工作者线程资源回收了 但是 ID 没改

    pthread_cleanup_pop(0); /* 取消清理函数 */
    thread_Exit(pool);  /* 线程正常退出清理函数 */
    return NULL;
}
...
void thread_Add(Threadpool *pool, void (*func)(void*), void *arg)
{
    ...
    //int queueSize = pool->queueSize;  ---------------------------------------------记住这个教训

    while (pool->queueSize == pool->queueCapacity && !pool ->shoutdown)
    {
